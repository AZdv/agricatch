...
def test_articles_returns_json(client, article):
...
def test_sort_by_accepts_a_known_field(client, article):
...
def test_sort_by_rejects_anything_else(client, article, value):
    response = client.get(reverse("tech:articles"), {"sort_by": value})
    assert response.status_code == 400


def test_non_numeric_days_future_is_a_bad_request_not_a_crash(client, article):
    assert client.get(reverse("tech:articles"), {"days_future": "abc"}).status_code == 400


def test_days_future_filters_to_the_window(client, article):
...
def test_importform_opens_for_staff(client):
