...
def test_hash_handles_a_null_time():
    article = Article.objects.create(**make(time=None))
    assert article.content_hash


def test_identical_content_collides_and_differing_content_does_not():
...
def test_build_hash_accepts_a_dict_or_an_instance():
    values = make()
    assert Article.build_hash(values) == Article.build_hash(Article(**values))


def test_reimporting_the_same_article_updates_rather_than_duplicates():
    values = make()
