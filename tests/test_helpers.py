...
def test_extension_functions_are_usable_from_xpath() -> None:
...
def test_findzipcode_returns_every_match() -> None:
    doc = etree.fromstring("<r><p>Berlin 10115 and 10247</p></r>")
    assert doc.xpath("findzipcode(//p/text())") == ["10115", "10247"]


def test_iter_functions_exposes_names_without_the_prefix() -> None:
...
def test_slugify_can_skip_the_aggressive_pass() -> None:
    assert text_helpers.slugify("Grüße!", extra_clean=False) == "gruesse!"


def test_replace_parameters_expands_the_crawl_date() -> None:
    when = datetime.date(2024, 3, 9)
    assert text_helpers.replace_parameters("/e/%Y-%m-%d", when) == "/e/2024-03-09"


def test_relative_url_to_absolute_handles_both_forms() -> None:
